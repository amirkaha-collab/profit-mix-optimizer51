...
def _alloc_plot(r: Dict):
    ...
    fig.update_layout(
        height=180, margin=dict(l=0,r=10,t=0,b=0),
...
def _render_reco_card(r: Dict, title: str, primary: bool = False, manager_names: Optional[List[str]] = None):
    ...
    with c1:
        ...
        _safe_plotly(_manager_donut(mgr_break))
    with c2:
        st.caption("תמהיל אפיקי השקעה")
        _safe_plotly(_alloc_plot(r))
    with c3:
        kpis = [
...
with tab2:
    ...
    if res is None:
        ...
        if not recs:
            ...
            _render_reco_card(weighted, "חלופה משוקללת", primary=True, manager_names=manager_names)

            # Secondary recommendations (side-by-side, less prominent)
            c1, c2 = st.columns(2)
            with c1:
                _render_reco_card(accurate, "החלופה המדויקת ביותר", primary=False, manager_names=manager_names)
            with c2:
                _render_reco_card(sharpe, "החלופה עם השארפ הטוב ביותר", primary=False, manager_names=manager_names)
